...
public sealed partial class AsyncRelayCommand<T> : AsyncDelegateCommand<T>
{
    public AsyncRelayCommand(Func<T, Task> executeMethod) : base(executeMethod)
    {
    }

    public AsyncRelayCommand(Func<T, CancellationToken, Task> executeMethod) : base(executeMethod)
    {
    }

    public AsyncRelayCommand(Func<T, Task> executeMethod, Func<T, bool> canExecuteMethod) : base(executeMethod, canExecuteMethod)
    {
    }

    public AsyncRelayCommand(Func<T, CancellationToken, Task> executeMethod, Func<T, bool> canExecuteMethod) : base(executeMethod, canExecuteMethod)
    {
    }
}
/// <summary>
...
//         this.execute = (parameter, cancellationToken) => execute(parameter);
...
//         else
//         {
//             await this.execute(parameter, CancellationToken.None);
//         }
//     }
//
//     /// <summary>
...
//
//     /// <summary>
//     /// 执行<see cref="ICommand.Execute(object)"/>方法。
//     /// </summary>
//     /// <param name="parameter">
//     /// 命令使用的数据。如果命令不需要传递数据，可以将此对象设置为<see langword="null"/>。
//     /// </param>
...
//
//         // 如果参数不是T类型，则命令不能执行
//         throw new InvalidOperationException($"参数 {parameter} 不能转换为类型 {typeof(T)}");
//     }
//
//     /// <summary>
//     /// 异步执行<see cref="IAsyncRelayCommand.ExecuteAsync(object)"/>方法。
//     /// </summary>
//     /// <param name="parameter">
//     /// 命令使用的数据。如果命令不需要传递数据，可以将此对象设置为<see langword="null"/>。
//     /// </param>
//     /// <returns>表示异步操作的任务。</returns>
//     async Task IAsyncRelayCommand.ExecuteAsync(object? parameter)
//     {
//         // 处理null情况
//         if (parameter is null)
//         {
...
//     /// <param name="parameter">命令使用的数据。</param>
//     /// <returns>表示异步操作的任务。</returns>
//     private async Task ExecuteConcurrentAsync(T? parameter)
//     {
//         // 如果已经有一个任务在运行，直接返回
...
//
//         Task executionTask = this.execute(parameter, cancellationToken);
//
